...
func (sc *ingressSource) endpointsFromTemplate(ing *v1.Ingress) ([]*endpoint.Endpoint, error) {
...
func (sc *ingressSource) filterByAnnotations(ingresses []*v1.Ingress) ([]*v1.Ingress, error) {
...
func (sc *ingressSource) setResourceLabel(ingress *v1.Ingress, endpoints []*endpoint.Endpoint) {
	for _, ep := range endpoints {
		ep.Labels[endpoint.ResourceLabelKey] = fmt.Sprintf("ingress/%s/%s", ingress.Namespace, ingress.Name)
	}
}

func (sc *ingressSource) setDualstackLabel(ingress *v1.Ingress, endpoints []*endpoint.Endpoint) {
...
func endpointsFromIngress(ing *v1.Ingress, ignoreHostnameAnnotation bool, ignoreIngressTLSSpec bool, ignoreIngressRulesSpec bool) []*endpoint.Endpoint {
	ttl, err := getTTLFromAnnotations(ing.Annotations)
	if err != nil {
		log.Warn(err)
	}
...
func targetsFromIngressStatus(status v1.IngressStatus) endpoint.Targets {
